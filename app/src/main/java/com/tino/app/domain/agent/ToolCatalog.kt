...
object TinoToolCatalog {
    val all: Set<TinoToolDescriptor> = setOf(
        ...
        TinoToolDescriptor(
            ...
            "suppliers.list",
            emptySet(),
            sourceOfTruth = "SupplierRepository / Room",
            a2uiComponent = "supplier_summary",
            capabilityId = TinoCapabilityId.LIST_SUPPLIERS,
        ),
        TinoToolDescriptor(
        ...
        TinoToolDescriptor(
            ...
            sourceOfTruth = "CustomerRepository / Room",
            entityResolution = TinoEntityResolution.CUSTOMER,
            a2uiComponent = "customer_contact",
            capabilityId = TinoCapabilityId.GET_CUSTOMER_CONTACT,
        ),
        TinoToolDescriptor(
            ...
            risk = TinoToolRisk.MEDIUM,
            sourceOfTruth = "CustomerRepository / Room",
            a2uiComponent = "action_confirmation",
            capabilityId = TinoCapabilityId.CREATE_CUSTOMER,
        ),
        TinoToolDescriptor(
            TinoToolId.PRODUCT_PRICE_UPDATE,
            "product.price.update",
            setOf("product_ref", "new_price_cents"),
            mode = TinoToolMode.PREPARE_ONLY,
            risk = TinoToolRisk.HIGH,
            sourceOfTruth = "ProductRepository / Room",
            entityResolution = TinoEntityResolution.PRODUCT,
            a2uiComponent = "price_change_preview",
            capabilityId = TinoCapabilityId.CHANGE_PRODUCT_PRICE,
        ),
        TinoToolDescriptor(
            TinoToolId.STOCK_ENTRY,
            "stock.entry",
            setOf("product_ref", "quantity", "unit_cost_cents", "supplier_ref"),
            mode = TinoToolMode.PREPARE_ONLY,
            risk = TinoToolRisk.HIGH,
            sourceOfTruth = "CommerceRepository / Room",
            entityResolution = TinoEntityResolution.MULTIPLE,
            a2uiComponent = "stock_entry_preview",
            capabilityId = TinoCapabilityId.REGISTER_STOCK_ENTRY,
        ),
        TinoToolDescriptor(
